...
contract OutboundQuoteScript is Script, LZConfig {
    ...
    uint32 internal srcEid = EID_ETHEREUM; // Source this from src/LZConfig.sol
    ...
    function run() public {
        ...
        console2.log(LibString.concat("------- ", deployment.chain.concat(" Outbound Quote Estimations -------")));
        console2.log("");
        console2.log(LibString.concat("Number of destination chains: ", uint256(_eids.length - 1).toString()));
        ...
        for (uint256 i; i < _eids.length; ++i) {
            if (_eids[i] == srcEid) continue;
            Deployment memory destination = deployments[_eids[i]];
            (uint256 nativeFee, uint256 lzTokenFee) = quoter.quote(uint32(_eids[i]), message, options, payInLzToken);
